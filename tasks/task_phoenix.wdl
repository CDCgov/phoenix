...
task phoenix {
  ...
  command <<<
    ...
    if nextflow run cdcgov/phoenix -plugins nf-google@1.1.3 -profile terra -r v1.1.1 -entry PHOENIX --terra true --input ../sample.csv --tmpdir $TMPDIR --max_cpus ~{cpu} --max_memory '~{memory}.GB' --kraken2db ~{kraken2db}; then
