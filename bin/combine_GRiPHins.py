...
def split_centar_df(centar, excel):
    ...
    if centar == True:
        ...
        columns_between = columns[start_index:end_index]
...
def update_centar_columns(centar_df_column_names_final, centar_df_column_names):
    ...
    return centar_df_column_names_final, total_unique_columns
...
def main():
    ...
    if args.griphin_list != False:
        ...
        for count, next_file in enumerate(griphin_files[1:], start=1):
            # checking 
            combined_df_qc, combined_df_ar, combined_df_pf, combined_df_hv, phoenix, shiga, centar, ordered_centar_df, centar_df_lens, centar_df_column_names = read_excel(next_file, phoenix_final, combined_df_qc, ordered_centar_df)
        ...
        combined_df_qc, combined_df_ar, combined_df_pf, combined_df_hv, phoenix_final, shiga_final, centar_final, ordered_centar_df, centar_df_lens_final = read_excels(args.griphin_new, args.griphin_old)
    #print(centar_df_lens_final)
    #print(centar_df_column_names_final)
    ...
    #write excel sheet
    write_to_excel(args.set_coverage, output_file, final_df, qc_max_col, ar_max_col, pf_max_col, hv_max_col, columns_to_highlight, final_ar_df, pf_db, ar_db, hv_db, phoenix_final, shiga_final, centar_final, centar_df_lens_final, ordered_centar_df)
