...
def parseArgs(args=None):
    parser = argparse.ArgumentParser(
        description="Script to generate a combined NCBI summary excel sheet"
    )
    parser.add_argument(
        "--biosample_output",
        dest="biosample_output",
        required=False,
        default="BiosampleAttributes_Microbe.1.0.xlsx",
        help="prefix for biosample final file.",
    )
    parser.add_argument(
        "--sra_output",
        dest="sra_output",
        required=False,
        default="Sra_Microbe.1.0.xlsx",
        help="prefix for sra final file.",
    )
    parser.add_argument(
...
def combine_second_rows(excel_files, final_file):
    ...
    invalid_files = [
        ...
        if pd.read_excel(file, header=0, nrows=1).isna().all().all()
    ]
    ...
    for file in valid_excel_files:
        ...
        combined_second_rows = pd.concat(
            [df, combined_second_rows], ignore_index=True, axis=0
        )
    ...
    combined_second_rows.to_excel(
        final_file, index=False, header=list(header_df.columns)
    )
...
def main():
    ...
    if len(biosample_list) != 0:
        print(
            str(len(biosample_list))
            + " BiosampleAttributes_Microbe.1.0.xlsx files were found using BiosampleAttributes_*_Microbe.1.0.xlsx!"
        )
        ...
        print(
            "No BiosampleAttributes_Microbe.1.0.xlsx files were found, you need to pass at least one!"
        )
        exit()
    if len(sra_list) != 0:
        print(
            str(len(sra_list))
            + " Sra_Microbe.1.0.xlsx files were found using Sra_*_Microbe.1.0.xlsx!"
        )
        ...
        print(
            "No Sra_Microbe.1.0.xlsx files were found, you need to pass at least one!"
        )
        exit()
