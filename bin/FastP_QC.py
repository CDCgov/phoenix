...
def parse_cmdline():
    ...
    parser = ArgumentParser(
        prog="FastP_QC.py", description="""Script collects stats from fastp jsons."""
    )
    parser.add_argument(
        "-t",
        "--trimmed_json",
    ...
    parser.add_argument(
        "-n", "--name", dest="name", action="store", required=True, help="Sample name"
    )
    parser.add_argument(
...
def FastP_QC_before(input_json, output_file, name):
    ...
    Out.write(
        "Name\tR1[reads]\tR1[bp]\tR2[reads]\tR2[bp]\tQ20_Total_[bp]\tQ30_Total_[bp]\tQ20_R1_[bp]\tQ20_R2_[bp]\tQ20_R1_[%]\tQ20_R2_[%]\tQ30_R1_[bp]\tQ30_R2_[bp]\tQ30_R1_[%]\tQ30_R2_[%]\tTotal_Sequenced_[bp]\tTotal_Sequenced_[reads]\n"
    )
    f = open(input_json)
    data = json.load(f)
    f.close()
    ...
    if Decimal(Info1["total_bases"]) == 0:
        Q20_R1_percent, Q20_R2_percent, Q30_R1_percent, Q30_R2_percent = (
            str(0) for i in range(4)
        )
    else:
        Q20_R1_percent = str(
            Decimal(Info1["q20_bases"]) / Decimal(Info1["total_bases"])
        )
        Q30_R1_percent = str(
            Decimal(Info1["q30_bases"]) / Decimal(Info1["total_bases"])
        )
        Q20_R2_percent = str(
            Decimal(Info2["q20_bases"]) / Decimal(Info2["total_bases"])
        )
        Q30_R2_percent = str(
            Decimal(Info2["q30_bases"]) / Decimal(Info2["total_bases"])
        )
...
def FastP_QC_after(input_trimmed_json, input_singles_json, output_file, name):
    ...
    Out = open(output_file, "w")
    Out.write(
        "Name\tR1[reads]\tR1[bp]\tR2[reads]\tR2[bp]\tUnpaired[reads]\tUnpaired[bps]\tQ20_Total_[bp]\tQ30_Total_[bp]\tQ20_R1_[bp]\tQ20_R2_[bp]\tQ20_unpaired[bp]\tQ20_R1_[%]\tQ20_R2_[%]\tQ20_unpaired[%]\tQ30_R1_[bp]\tQ30_R2_[bp]\tQ30_unpaired[bp]\tQ30_R1_[%]\tQ30_R2_[%]\tQ30_unpaired[%]\tTotal_Sequenced_[bp]\tPaired_Sequenced_[reads]\tTotal_Sequenced_[reads]\n"
    )
    ...
    Total_Sequenced_reads = str(
        int(Trimmed_Sequenced_reads) + int(Singles_Sequenced_reads)
    )
    ...
    Line = (
        ...
        + Q30_R2_percent
        + "\t"
        + Q30_unpaired_percent
        + "\t"
        + Total_Sequenced_bp
        + "\t"
        + Trimmed_Sequenced_reads
        + "\t"
        + Total_Sequenced_reads
    )
    Out.write(Line)
