...
def get_raw_stats(stats):
    with open(stats) as f:
        ...
        Q20_percent = str(
            round(float(lines[4].strip("\n").replace("q20 percents: ", "")) / 100, 4)
        )  # need 0.XXXX notation for pipeline_stats_writer.sh
        Q30_bp = int(lines[3].strip("\n").replace("q30 bases: ", ""))
        Q30_percent = str(
            round(float(lines[5].strip("\n").replace("q30 percents: ", "")) / 100, 4)
        )  # need 0.XXXX notation for pipeline_stats_writer.sh
...
def all_raw_stats(r1_stats, r2_stats, name):
    ...
    raw_R1_reads, raw_R1_bases, Q20_R1_bp, Q20_R1_percent, Q30_R1_bp, Q30_R1_percent = (
        get_raw_stats(r1_stats)
    )
    raw_R2_reads, raw_R2_bases, Q20_R2_bp, Q20_R2_percent, Q30_R2_bp, Q30_R2_percent = (
        get_raw_stats(r2_stats)
    )
    write_raw_stats(
        raw_R1_reads,
        raw_R1_bases,
