...
def parseArgs(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Script to generate template NCBI excel sheets for samples in PHoeNIx run"
    )
    parser.add_argument(
        "-i",
        "--input",
        ...
        default=None,
        required=False,
        help="A GRiPHin_samplesheet. If not passed then you need to pass a directory.",
    )
    parser.add_argument("--biosample-type", type=str, required=True)
    parser.add_argument("-o", "--output", type=str, required=True)
    parser.add_argument(
        "-m", "--microbe", dest="microbe_example", type=str, required=True
    )
    parser.add_argument("-s", "--sra", dest="sra_metadata", type=str, required=True)
    parser.add_argument(
        "-g", "--griphin_summary", dest="griphin_summary", type=str, required=True
    )
    parser.add_argument(
        "-b", "--osii_bioprojects", dest="osii_bioprojects", type=str, required=True
    )
    parser.add_argument(
        "-d",
        "--directory",
        ...
        type=str,
        required=False,
        help="Pass a PHoeNIx output directory rather than a GRiPHin_samplesheet.",
    )
    parser.add_argument(
...
def get_isolate_dirs(directory, griphin_summary):
    ...
    ext = [".tsv", ".csv", ".xlsx"]
    filtered_dirs = [
        val for val in directories if not val.endswith((".tsv", ".csv", ".xlsx"))
    ]
    ...
    dirs_cleaned = [
        val
        for val in filtered_dirs
        if not any(str(bad_val) in str(val) for bad_val in skip_list)
    ]  # remove unwanted files
    if len(dirs_cleaned) == 0:
        print(
            "No samples in this run passed QC in the GRiPHin Summary, thus no NCBI sheet will be written."
        )
        exit()
...
def load_bio_projects(sample_type, isolate_list_path, microbe_example):
    ...
    if "Microbe" in sample_type or "microbe" in sample_type:
        ...
        df = pd.read_excel(
            microbe_example, header=0, sheet_name="Microbe.1.0", comment="#"
        )
...
def ncbi_excel_loader(biosample_example_path, isolate_list_path):
    ...
    index = df.index[
        df.apply(
            lambda x: all(
                keyword in str(x.iloc[:]) for keyword in ["sample_name", "sample_title"]
            ),
            axis=1,
        )
    ]
...
def load_sra(ncbi_project_type, isolate_list_path, sra_metadata):
    ...
    if "Microbe" in ncbi_project_type or "microbe" in ncbi_project_type:
        ...
        columns = (
            pd.read_excel(sra_metadata, header=None, sheet_name="SRA_data")
            .iloc[0]
            .tolist()
        )
        for isolate in isolate_names:
...
def check_project(one_isoalte_taxo, bioprojects_taxo):
    ...
    if (
        one_isoalte_taxo["G"] + "(G)" + " " + one_isoalte_taxo["s"] + "(s)"
    ) in bioprojects_taxo.keys():
        flag = True
        return one_isoalte_taxo["G"] + "(G)" + " " + one_isoalte_taxo["s"] + "(s)"
    if (one_isoalte_taxo["G"] + "(G)") == "Staphylococcus(G)" and (
        one_isoalte_taxo["s"] + "(s)"
    ) != "aureus(s)":
        flag = True
...
def fill_meta_values(
    meta_info_biosample, isolate_taxs, mlst_info_isolate, bioprojects_taxo
):
    project = ""
    for isolate_name in list(isolate_taxs.keys()):
        ...
        if check_project(isolate_taxs[isolate_name], bioprojects_taxo) != "":
            ...
            meta_info_biosample[isolate_name].sampleContent["bioproject_accession"] = (
                bioprojects_taxo[project]
            )
        else:
            meta_info_biosample[isolate_name].sampleContent["bioproject_accession"] = ""
        meta_info_biosample[isolate_name].sampleContent["*organism"] = (
            isolate_taxs[isolate_name]["G"] + " " + isolate_taxs[isolate_name]["s"]
        )
        ...
        meta_info_biosample[isolate_name].sampleContent[
            "*sample_type"
        ] = "whole organism"
        meta_info_biosample[isolate_name].sampleContent["MLST"] = mlst_info_isolate[
            isolate_name
        ]
...
def base_output(output_path, sra, sra_columns, bio_attribute, biosample_columns):
    ...
    df_biosample = pd.DataFrame(tools.purify_dict(bio_attribute)).T.reset_index(
        drop=True
    )
...
def add_disclaimer(df, input_excel, input_sheet_name):
    ...
    with pd.ExcelWriter(input_excel, engine="xlsxwriter") as writer:
        ...
        orange_format = workbook.add_format(
            {"color": "orange", "bold": True, "text_wrap": True}
        )
...
including from the Antimicrobial Resistance Laboratory Network (AR Lab Network), state public health labs, surveillance programs, and outbreaks. For inquiries about the appropriate BioProject location for your data, please contact HAISeq@cdc.gov."""
        ...
        if input_sheet_name == "SRA_data":
            worksheet.merge_range(
                "A" + str(num_rows + 1) + ":J" + str(num_rows + 4),
                sra_delete_warning,
                orange_format,
            )
            worksheet.merge_range(
                "A" + str(num_rows + 5) + ":J" + str(num_rows + 8),
                disclaimer_text,
                red_format,
            )
        else:
            worksheet.merge_range(
                "A" + str(num_rows + 1) + ":J" + str(num_rows + 6),
                biosample_delete_warning,
                orange_format,
            )
            worksheet.merge_range(
                "A" + str(num_rows + 7) + ":J" + str(num_rows + 10),
                disclaimer_text,
                red_format,
            )
...
def base_function(
    ...
    biosample, biosample_columns = load_bio_projects(
        sample_type, determined_isolate_full_path, microbe_example
    )
    tax_info_isolate = retrieve_taxo_mlst.retrieve_taxo(determined_isolate_full_path)
    mlst_info_isolate = retrieve_taxo_mlst.retrieve_mlst_nonovel(
        determined_isolate_full_path
    )
    seq_machine_isolate = retrieve_taxo_mlst.retrieve_instrument_model(
        determined_isolate_full_path
    )
...
def manage_functions(
    ...
    if (
        input_file != None
    ):  # If a griphin file is not passed then assume the files are in the directory where script is being run.
        if directory != None:
            exit("Pick Either -i or -d to pass. You can't have both.")
        else:
            if check_input(input_file) == "csv":
                isolate_full_path_griphin = FileLoader().get_full_path_griphin(
                    input_file
                )
                base_function(
                    isolate_full_path_griphin,
                    sample_type,
                    output,
                    microbe_example,
                    sra_metadata,
...
def main():
    ...
    manage_functions(
        ...
        args.biosample_type,
        args.output,
        args.microbe_example,
        args.sra_metadata,
        args.osii_bioprojects,
        args.directory,
        args.griphin_summary,
    )
    end_time = time.time()
