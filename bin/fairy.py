...
def parseArgs(args=None):
    ...
    parser.add_argument(
        "-t", "--trimd_read", dest="trimd_read", default=None, required=False
    )
    parser.add_argument(
        "-b",
        "--busco",
        ...
        default=False,
        action="store_true",
        help="Pass to make true for -entry cdc pipelines",
    )  # Need this for when you call -entry CDC_PHOENIX or CDC_SCAFFOLDS, but spades fails
    parser.add_argument(
...
def reads_compare(read_file, trimd_file, filename, busco):
    ...
    if (
        trimd_file != None
    ):  # if you have reached the trim step then read pairs were prior to trimming and we will check there are reads post trimming
        ...
        )  # essentially should never show up
        error = "There are 0 reads in R1/R2 after trimming!"

        if (
            aggr_trimd_stats["R1[reads]"].values[0] > 0
            and aggr_trimd_stats["R2[reads]"].values[0] > 0
        ):
            outcome = approved
            ...
            (
                ...
                raw_Q30_R2_rounded,
                raw_orphaned_reads,
            ) = get_read_stats(aggr_read_stats, "false")
            (
                trimd_length_R1,
                trimd_length_R2,
                trimd_reads,
                trimd_pairs,
                trimd_Q30_R1_rounded,
                trimd_Q30_R2_rounded,
                trimd_orphaned_reads,
            ) = get_read_stats(aggr_trimd_stats, "true")
            warning_count = write_synopsis(
                prefix,
                busco,
                raw_length_R1,
                raw_length_R2,
...
def write_synopsis(
    ...
    with open(sample_name + ".synopsis", "a") as f:
        ...
        if raw_reads <= 1000000 and raw_reads >= 1:
            f.write(
                "RAW_READ_COUNTS               : WARNING : Low individual read count before trimming: "
                + str(raw_reads)
                + "("
                + raw_pairs
                + " paired reads)\n"
            ...
            f.write(
                "RAW_READ_COUNTS               : FAILED  : No individual read count before trimming: "
                + str(raw_reads)
                + "("
                + raw_pairs
                + " paired reads)\n"
            ...
            f.write(
                "RAW_READ_COUNTS               : SUCCESS : "
                + str(raw_reads)
                + " individual reads found in sample ("
                + raw_pairs
                + " paired reads)\n"
            )
        if raw_Q30_R1_rounded < 90:
            f.write(
                "RAW_Q30_R1%                   : WARNING : Q30_R1% at "
                + str(raw_Q30_R1_rounded)
                + "% (Threshold is 90%)\n"
            )
            warning_count = warning_count + 1
        else:
            f.write(
                "RAW_Q30_R1%                   : SUCCESS : Q30_R1% at "
                + str(raw_Q30_R1_rounded)
                + "% (Threshold is 90%)\n"
            )
        if raw_Q30_R2_rounded < 70:
            f.write(
                "RAW_Q30_R1%                   : WARNING : Q30_R1% at "
                + str(raw_Q30_R2_rounded)
                + "% (Threshold is 70%)\n"
            )
            warning_count = warning_count + 1
        else:
            f.write(
                "RAW_Q30_R2%                   : SUCCESS : Q30_R2% at "
                + str(raw_Q30_R2_rounded)
                + "% (Threshold is 70%)\n"
            )
        if trimd_file == None:
            f.write(
                "TRIMMED_R1                    : FAILED  : "
                + sample_name
                + "_trimmed_read_counts.txt not found. No trimming performed -- "
                + Error
            )
            f.write(
                "TRIMMED_R2                    : FAILED  : "
            ...
            f.write(
                "TRIMMED_FASTQs                : FAILED  : "
                + sample_name
                + "_trimmed_read_counts.txt not found. No trimming performed -- "
                + Error
            )
            f.write(
                "TRIMMED_READ_COUNTS           : FAILED  : "
                + sample_name
                + "_trimmed_read_counts.txt not found. No trimming performed -- "
                + Error
            )
            f.write(
                "TRIMMED_Q30_R1%               : FAILED  : "
                + sample_name
                + "_trimmed_read_counts.txt not found. No trimming performed -- "
                + Error
            )
            f.write(
                "TRIMMED_Q30_R2%               : FAILED  : "
                + sample_name
                + "_trimmed_read_counts.txt not found. No trimming performed -- "
                + Error
            )
            ...
            f.write(
                "TRIMMED_READ_COUNTS           : FAILED  : No individual read count after trimming: "
                + str(trimd_reads)
                + "("
                + trimd_pairs
                + "paired reads, "
            ...
            if trimd_Q30_R1_rounded < 90:
                f.write(
                    "TRIMMED_Q30_R1%           : WARNING : Q30_R1% at "
                    + str(trimd_Q30_R1_rounded)
                    + "% (Threshold is 90%)\n"
                )
                warning_count = warning_count + 1
            else:
                f.write(
                    "TRIMMED_Q30_R1%           : SUCCESS : Q30_R1% at "
                    + str(trimd_Q30_R1_rounded)
                    + "% (Threshold is 90%)\n"
                )
            if trimd_Q30_R2_rounded < 70:
                f.write(
                    "TRIMMED_Q30_R2%           : WARNING : Q30_R2% at "
                    + str(trimd_Q30_R2_rounded)
                    + "% (Threshold is 70%)\n"
                )
                warning_count = warning_count + 1
            else:
                f.write(
                    "TRIMMED_Q30_R2%           : SUCCESS : Q30_R2% at "
                    + str(trimd_Q30_R2_rounded)
                    + "% (Threshold is 70%)\n"
                )
        f.write(
            "KRAKEN2_READS                 : FAILED  : "
            + sample_name
            + ".kraken2_trimd.report.txt not found -- "
            + Error
        )
        f.write(
            "KRONA_READS                   : FAILED  : kraken2 reads do not exist -- "
            + Error
        )
        f.write(
            "KRAKEN2_CLASSIFY_READS        : FAILED  : There are no classified reads -- "
            + Error
        )
        ...
        if busco == True:
            f.write(
                "SRST2                         : FAILED  : "
                + sample_name
                + "__fullgenes__*.txt file does not exist. SRST2 was not run. -- "
                + Error
            )
        f.write(
            "SCAFFOLD_TRIM                 : FAILED  : "
            + sample_name
            + ".filtered.scaffolds.fa.gz not found -- "
            + Error
        )
        f.write(
            "KRAKEN2_ASMBLD                : FAILED  : "
            + sample_name
            + ".kraken2_asmbld.report.txt not found -- "
            + Error
        )
        f.write(
            "KRONA_ASMBLD                  : FAILED  : kraken2 unweighted not performed -- "
            + Error
        )
        f.write(
            "KRAKEN2_CLASSIFY_ASMBLD       : FAILED  : kraken2 assembly not performed -- "
            + Error
        )
        f.write(
            "KRAKEN2_ASMBLD_CONTAM         : FAILED  : kraken2 assembly not performed -- "
            + Error
        )
        f.write(
            "KRAKEN2_WEIGHTED              : FAILED  : kraken2 weighted not performed -- "
            + Error
        )
        f.write(
            "KRONA_WEIGHTED                : FAILED  : kraken2 weighted not performed -- "
            + Error
        )
        f.write(
            "KRAKEN2_CLASSIFY_WEIGHTED     : FAILED  : kraken2 weighted not performed -- "
            + Error
        )
        f.write(
            "KRAKEN2_WEIGHTED_CONTAM       : FAILED  : kraken2 weighted not performed -- "
            + Error
        )
        f.write(
            "QUAST                         : FAILED  : "
            + sample_name
            + "_report.tsv does not exist. QUAST was not run, no assembly -- "
            + Error
        )
        f.write(
        ...
        f.write(
            "ASSEMBLY_RATIO(SD)            : FAILED  : No Ratio File exists -- " + Error
        )
        f.write(
            "COVERAGE                      : FAILED  : No trimmed reads to review for coverage -- "
            + Error
        )
        if busco == True:
            f.write(
                "BUSCO                         : FAILED  : BUSCO was not performed -- "
                + Error
            )
        f.write(
            "FASTANI_REFSEQ                : FAILED  : FASTANI was not performed -- "
            + Error
        )
        f.write(
            "MLST                          : FAILED  : MLST was not performed -- "
            + Error
        )
        f.write(
            "GAMMA_AR                      : FAILED  : GAMMA_AR was not performed -- "
            + Error
        )
        f.write(
            "PLASMID_REPLICONS             : FAILED  : GAMMA was not performed -- "
            + Error
        )
        f.write(
            "HYPERVIRULENCE                : FAILED  : GAMMA was not performed -- "
            + Error
        )
        f.write(
            "Auto PASS/FAIL                : FAILED  : FASTQs couldn't be unzipped!\n"
        )
        f.write(
            "---------- " + sample_name + " completed as " + status + " ----------\n"
        )
        f.write("Unequal number of reads in R1/R2. Please fix and rerun PHoeNIx.\n")
        f.write(
            "PHoeNIx will only analyze FASTQ files that have an equal number of reads.\n"
        )
        f.write(
