...
def parseArgs(args=None):
    parser = argparse.ArgumentParser(
        description="Script to generate a PhoeNix summary line"
    )
    parser.add_argument("-n", "--name", required=True, help="sequence name")
    parser.add_argument(
        "-t", "--trimmed", required=False, help="QC data file for trimmed reads"
    )
    ...
    parser.add_argument(
        ...
        dest="mutations",
        required=False,
        help="Mutation file from AMRFinder",
    )
    ...
    parser.add_argument(
        ...
        dest="fastani",
        required=False,
        help="Fastani file or empty placeholder.",
    )
    parser.add_argument(
        "-k",
        "--kraken_trim",
        dest="trimd_kraken",
        required=False,
        help="trimd_summary.txt from kraken2",
    )
    parser.add_argument(
        "-s",
        "--stats",
        dest="stats",
        required=False,
        help="Pipeline Stats file synopsis file",
    )
    parser.add_argument(
        "-e",
        "--extended_qc",
            ...
            if DB_ID in Scheme_list[0]:
                ...
                for i in range(0, len(Scheme_list[0])):
                    if DB_ID == Scheme_list[0][i]:
                        ...
                        if (
                            Scheme != "-" and "Novel" not in Scheme
                ...
                if (
                    Scheme != "-"
                    and Scheme != "Novel_allele"
                    and Scheme != "Novel_profile"
                ):
...
def Isolate_Line(
    ...
    try:
        taxa_source, percent_match, Species, fastani_coverage = Get_Taxa_Source(
            Taxa, fastani
        )
    except:
        taxa_source = "Unknown"
...
def Isolate_Line_File(
    ...
    with open(out_file, "w") as f:
        Line, busco, fastani = Isolate_Line(
            Taxa,
            fastani,
            ID,
            trimmed_counts,
        ...
        if busco == True:
            f.write(
                "ID\tAuto_QC_Outcome\tWarning_Count\tEstimated_Coverage\tGenome_Length\tAssembly_Ratio_(STDev)\t#_of_Scaffolds_>500bp\tGC_%\tBUSCO\tBUSCO_DB\tSpecies\tTaxa_Confidence\tTaxa_Coverage\tTaxa_Source\tKraken2_Trimd\tKraken2_Weighted\tMLST_Scheme_1\tMLST_1\tMLST_Scheme_2\tMLST_2\tGAMMA_Beta_Lactam_Resistance_Genes\tGAMMA_Other_AR_Genes\tAMRFinder_Point_Mutations\tHypervirulence_Genes\tPlasmid_Incompatibility_Replicons\tAuto_QC_Failure_Reason\n"
            )
        else:
            f.write(
                "ID\tAuto_QC_Outcome\tWarning_Count\tEstimated_Coverage\tGenome_Length\tAssembly_Ratio_(STDev)\t#_of_Scaffolds_>500bp\tGC_%\tSpecies\tTaxa_Confidence\tTaxa_Coverage\tTaxa_Source\tKraken2_Trimd\tKraken2_Weighted\tMLST_Scheme_1\tMLST_1\tMLST_Scheme_2\tMLST_2\tGAMMA_Beta_Lactam_Resistance_Genes\tGAMMA_Other_AR_Genes\tAMRFinder_Point_Mutations\tHypervirulence_Genes\tPlasmid_Incompatibility_Replicons\tAuto_QC_Failure_Reason\n"
            )
        f.write(Line)
