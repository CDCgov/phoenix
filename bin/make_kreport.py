...
class Tree(object):
    ...
    def __init__(
        self, taxid, name, level_rank, level_num, p_taxid, parent=None, children=None
    ):
        self.taxid = taxid
        self.name = name
...
def main():
    ...
    for line in k_file:
        ...
        if read_count % 1000 == 0:
            sys.stdout.write(
                "\r\t%0.3f million reads processed" % float(read_count / 1000000.0)
            )
            sys.stdout.flush()
    ...
    k_file.close()
    sys.stdout.write(
        "\r\t%0.3f million reads processed\n" % float(read_count / 1000000.0)
    )
    sys.stdout.flush()
    ...
    while len(parse_nodes) > 0:
        ...
        o_file.write(
            "%6.2f\t" % (float(taxid2allcounts[curr_tid]) / float(read_count) * 100)
        )
