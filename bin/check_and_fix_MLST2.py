...
def download_MLST_files(tax_to_download, work_dir):
	...
	if path.isfile(work_dir+"/"+species_name_underscores+"_pull_dates.txt"):
		with open(work_dir+"/"+species_name_underscores+"_pull_dates.txt",'a+') as pull_file:
		...
		with open(work_dir+"/"+species_name_underscores+"_pull_dates.txt",'w') as pull_file:
...
#			print("\n".join(original_items))
	...
	#					print(":", original_items[allele])
					...
					expanded_allele_list.append([sample,db_name, original_type, len(allele_names), allele_names, [], "standard"])
	...
	#					print(expanded_allele_list)
					...
					expanded_allele_list.append([sample,db_name, "-", 1, ["-"], ["-"], "standard"])
			...
			#allele_list=[['1'], ['3'], ['189','3'], ['2'], ['2'], ['96','107'], ['3']]
				...
				if len(original_items) > 7:
					...
					expanded_allele_list.append([sample,db_name, original_type, len(allele_names), allele_names, [], "srst2"])
	...
	#									print("After adding - ", j)
					...
					expanded_allele_list.append([sample,db_name, "-", 1, ["-"], ["-"], "srst2"])
	...
	for i in checked_schemes:
		print(i[1], i[5][2])
		if i[1] == "abaumannii" or i[1] == "abaumannii(Oxford)" or i[1] == "Acinetobacter baummannii#1":
			if str(i[5][2]) == "182" or str(i[5][2])== "189":
				print("FAKE NEWS!!!")
				i[2] = str(i[2])+"-PARALOG"
			i[1] = "abaumannii(Oxford)"
		elif i[1] == "abaumannii_2":
