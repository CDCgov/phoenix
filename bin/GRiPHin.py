...
def parseArgs(args=None):
    parser = argparse.ArgumentParser(
        description="Script to generate a PhoeNix summary excel sheet."
    )
    parser.add_argument(
        "-s",
        "--samplesheet",
        ...
        required=False,
        dest="samplesheet",
        help="PHoeNIx style samplesheet of sample,directory in csv format. Directory is expected to have PHoeNIx stype output.",
    )
    parser.add_argument(
        "-d",
        "--directory",
        ...
        required=False,
        dest="directory",
        help="If a directory is given rather than samplesheet GRiPHin will create one for all samples in the directory.",
    )
    parser.add_argument(
        "-c",
        "--control_list",
        required=False,
        dest="control_list",
        help='CSV file with a list of sample_name,new_name. This option will output the new_name rather than the sample name to "blind" reports.',
    )
    parser.add_argument(
        "-a",
        "--ar_db",
        ...
        required=True,
        dest="ar_db",
        help="AR Gene Database file that is used to confirm srst2 gene names are the same as GAMMAs output.",
    )
    parser.add_argument(
        "-o",
        "--output",
        ...
        required=False,
        dest="output",
        help="Name of output file default is GRiPHin_Summary.xlsx.",
    )
    parser.add_argument(
        "--coverage",
        default=30,
        required=False,
        dest="set_coverage",
        help="The coverage cut off default is 30x.",
    )
    parser.add_argument(
        "--scaffolds",
        dest="scaffolds",
        default=False,
        action="store_true",
        help="Turn on with --scaffolds to keep samples from failing/warnings/alerts that are based on trimmed data. Default is off.",
    )
    parser.add_argument(
        "--phoenix",
        dest="phoenix",
        ...
        action="store_true",
        required=False,
        help="Use for -entry PHOENIX rather than CDC_PHOENIX, which is the default.",
    )
    parser.add_argument(
...
def Get_Parent_Folder(directory):
    ...
    parent_folder = os.path.split(
        os.path.split(os.path.split(os.path.split(directory)[0])[0])[0]
    )[0]
...
def get_Q30(trim_stats, raw_stats):
    ...
    Trim_Q30_R1_percent = round(data_df["Q30_R1_[%]"] * 100, 2)[
        0
    ]  # make percent and round to two decimals
    ...
    Q30_R1_percent = round(data_raw_df["Q30_R1_[%]"] * 100, 2)[
        0
    ]  # make percent and round to two decimals
...
def get_kraken_info(kraken_trim, kraken_wtasmbld, sample_name):
    ...
    try:
        ...
        Asmbld_kraken = (
            Asmbld_Genus
            + " ("
            + Asmbld_Genus_percent
            + ") "
            + Asmbld_Species
            + " ("
            + Asmbld_Species_percent
            + ")"
        )
        # guess what the mlst scheme is to check later
    ...
    return (
        ...
        Asmbld_kraken,
        Asmbld_Genus_percent,
        Trim_unclassified_percent,
        Asmbld_unclassified_percent,
        scheme_guess_kraken_wt,
        scheme_guess_kraken_trimd,
    )
    ...
    with open(gc_file, "r") as f:
        for line in f:
            if "Species_GC_StDev:" in line:
                if (
                    "Not calculated on species with n<10 references" in line
                    or "No Match Found" in line
                ):
                    gc_stdev = "NA"
                ...
                if "No Match Found" in line:
                    ...
                    species_gc_mean = float(
                        (line.split("Species_GC_Mean: ", 1)[1]).strip()
                    )
            else:
                pass
    ...
    with open(asmbld_ratio, "r") as f:
        for line in f:
            ...
            if "Isolate_St.Devs:" in line:
                ...
                if (
                    stdev == "NA" or stdev == "No Match Found"
                ):  # handling making stdev a float only if its a number
                    pass
                else:
                    stdev = float(stdev)
            if "Ratio: " in line:
...
def compile_alerts(scaffolds_entry, coverage, assembly_stdev, gc_stdev):
    ...
    if str(assembly_stdev) == "NA":
        if str(gc_stdev) == "NA":
            alerts.append(
                "Assembly ratio and GC% STDev are N/A <10 genomes as reference"
            )
        else:
            alerts.append(
                "Open Github issue assembly ratio STDev is N/A but not GC%. This shouldn't happen."
            )
    elif str(gc_stdev) == "NA":
        alerts.append(
            "Open Github issue STDev is N/A for GC% and not assembly ratio. This shouldn't happen."
        )
    else:
        pass
    alerts = ", ".join(alerts)
...
def compile_warnings(
    ...
    scaffolds,
    gc_metrics,
    assembly_ratio_metrics,
    Trim_unclassified_percent,
    Wt_asmbld_unclassified_percent,
    kraken_trim_genus,
    ...
    if scaffolds_entry == False:
        ...
        if Trim_Q30_R1_per == "Unknown" or float(Trim_Q30_R1_per) < float(90.00):
            warnings.append(
                "Average Q30 of trimmed R1 reads <{:.2f}% ({:.2f}%)".format(
                    float(90.00), float(Trim_Q30_R1_per)
                )
            )
        if Trim_Q30_R2_per == "Unknown" or float(Trim_Q30_R2_per) < float(70.00):
            warnings.append(
                "Average Q30 of trimmed R2 reads <{:.2f}% ({:.2f}%)".format(
                    int(70.00), float(Trim_Q30_R2_per)
                )
            )
        if Trim_unclassified_percent == "trimmed" or float(
            Trim_unclassified_percent
        ) > float(30.00):
            warnings.append(">{:.2f}% unclassifed trimmed reads".format(int(30)))
        if len(kraken_trim_genus) >= 2:
            warnings.append(
                ">=2 genera had >{:.2f}% of reads assigned to them".format(int(25))
            )
        if float(Trim_Genus_percent) < float(70.00):
            warnings.append(
                "<70% of reads assigned to top genera hit ({:.2f}%)".format(
                    float(Trim_Genus_percent)
                )
            )
    else:
        pass
    if gc_metrics[0] != "NA" and gc_metrics[0] != "Unknown":
        ...
        if float(gc_metrics[1]) > (
            float(gc_metrics[3]) + float(gc_metrics[2])
        ):  # check that gc% is < 2.58 stdev away from mean gc of species
            warnings.append(
                "GC% >2.58 stdev away from mean GC of {:.2f}%.".format(
                    float(gc_metrics[3])
                )
            )
    if (
        scaffolds != "Unknown"
        and Wt_asmbld_unclassified_percent != "Unknown"
        and Asmbld_Genus_percent != "Unknown"
    ):
        ...
        if float(Asmbld_Genus_percent) < float(70.00):
            warnings.append(
                "<70% of weighted scaffolds assigned to top genera hit ({:.2f}%)".format(
                    float(Asmbld_Genus_percent)
                )
            )
    elif (
        scaffolds == "Unknown"
    ...
    if len(kraken_wtasmbld_genus) >= 2:
        warnings.append(
            ">=2 genera had >{:.2f}% of wt scaffolds assigned to them.".format(int(25))
        )
    if MLST_scheme_1 != "-" and not MLST_scheme_1.startswith(scheme_guess):
        if genus == "Enterobacter" and MLST_scheme_1 == "ecloacae":
            ...
            genus == "Shigella" or genus == "Escherichia"
        ) and MLST_scheme_1.startswith("ecoli"):
            pass
        else:
    ...
    if MLST_scheme_2 != "-" and not MLST_scheme_2.startswith(scheme_guess):
        if genus == "Enterobacter" and MLST_scheme_2 == "ecloacae":
            ...
            genus == "Shigella" or genus == "Escherichia"
        ) and MLST_scheme_2.startswith("ecoli"):
            pass
        else:
        ...
        with open(kraken_wtasmbld_report, mode="r", encoding="utf8", newline="\r") as f:
            for line in f:  # weird file so its really just one big long line
                ...
                if "\tunclassified" in split_line[0]:
                    unclass_amount = float(
                        split_line[0].split("\t")[1].replace(" ", "")
                    )
                else:
                    unclass_amount = 0
                for thing in split_line:
...
def Checking_auto_pass_fail(
    ...
    QC_result = (
        []
    )  # set as blank to begin with, need this to assign variable in QC_result == "FAIL": line
    ...
    with open(fairy_file, "r") as f:
        for line in f:
            if "FAILED CORRUPTION CHECK!" in line:
                ...
                QC_reason.append(
                    str(fastq_file_failure)
                    + " is corrupt and is unable to be unzipped."
                )
    ...
    f.close()
    if (
        scaffolds_entry == False
    ):  # if its not being used for scaffolds entry check estimated coverage otherwise don't
        if coverage == "Unknown" or int(coverage) < int(set_coverage):
            QC_result.append("FAIL")
            if (
                coverage == "Unknown"
            ):  # if else really only needed so you don't end up with "unknownx"
                QC_reason.append(
                    "coverage <" + str(set_coverage) + "x (" + str(coverage) + ")"
                )
            else:
                QC_reason.append(
                    "coverage <" + str(set_coverage) + "x (" + str(coverage) + "x)"
                )
    else:
        pass
    ...
    if (
        str(assembly_stdev) != "NA"
    ):  # have to have a second layer cuz you can't make NA a float, N/A means less than 10 genomes so no stdev calculated
        if (
            str(asmbld_ratio) == "Unknown"
        ):  # if there is no ratio file then fail the sample
            QC_result.append("FAIL")
...
def parse_gamma_ar(gamma_ar_file, sample_name, final_df):
    ...
    DB = (
        (gamma_ar_file.rsplit("/", 1)[-1]).replace(sample_name, "").rsplit("_")[1]
        + "_"
        + (gamma_ar_file.rsplit("/", 1)[-1]).replace(sample_name, "").rsplit("_")[2]
        + "([XNT/98AA/90]G:[98NT/90]S)"
    )
    percent_BP_IDs = np.floor(
        gamma_df["BP_Percent"] * 100
    ).tolist()  # round % to whole number
    percent_codon_IDs = np.floor(
        gamma_df["Codon_Percent"] * 100
    ).tolist()  # round % to whole number
    percent_lengths = np.floor(
        gamma_df["Percent_Length"] * 100
    ).tolist()  # round % to whole number
    conferred_resistances = (
        gamma_df["Gene"].str.split("__").str[4]
    )  # parse "Gene" column in gamma file to get conferred resistance out of gene name
    contig_numbers = (
        gamma_df["Contig"].str.replace(sample_name, "").str.split("_").str[1]
    )  # Parse "Contig" column in gamma file
    genes = (
        gamma_df["Gene"].str.split("__").str[2]
    )  # Parse "Gene" column in gamma file to get gene name and accession
    # loop through list of genes to combine with conferred resistance and make back into a pandas series
    column_name = [
        "{}_({})".format(gene, conferred_resistance)
        for gene, conferred_resistance in zip(genes, conferred_resistances)
    ]
    # loop through list of gamma info to combine into "code" for ID%/%cov:contig# and make back into a pandas series
    coverage = [
        "[{:.0f}NT/{:.0f}AA/{:.0f}:#{}]G".format(
            percent_BP_ID, percent_codon_ID, percent_length, contig_number
        )
        for percent_BP_ID, percent_codon_ID, percent_length, contig_number in zip(
            percent_BP_IDs, percent_codon_IDs, percent_lengths, contig_numbers
        )
    ]
    ...
    if bool(
        [
            percent_length
            for percent_length in percent_lengths
            if int(percent_length) < 90
        ]
    ):
        index_remove_postion = [
            n
            for n, percent_length in enumerate(percent_lengths)
            if int(percent_length) < 90
        ]  # get index for value removed to remove from other lists (values less than 90)
        percent_lengths = [
            percent_length
            for percent_length in percent_lengths
            if int(percent_length) >= 90
        ]  # filter list to remove values below cutoff (keep those greater than or equal to 90)
    ...
    if bool(
        [
            percent_codon_ID
            for percent_codon_ID in percent_codon_IDs
            if int(percent_codon_ID) < 98
        ]
    ):
        index_remove_postion = [
            n
            for n, percent_codon_ID in enumerate(percent_codon_IDs)
            if int(percent_codon_ID) < 98
        ]  # get index for value removed to remove from other lists (values less than 98)
        percent_codon_IDs = [
            percent_codon_ID
            for percent_codon_ID in percent_codon_IDs
            if int(percent_codon_ID) >= 98
        ]  # filter list to remove values below cutoff (keep those greater than or equal to 98)
...
def parse_gamma_hv(gamma_hv_file, sample_name, final_df):
    ...
    DB = (
        (gamma_hv_file.rsplit("/", 1)[-1]).replace(sample_name, "").rsplit("_")[1]
        + "_"
        + (gamma_hv_file.rsplit("/", 1)[-1])
        .replace(sample_name, "")
        .rsplit("_")[2]
        .strip(".gamma")
    )
    ...
    if df.empty:
        df = pd.DataFrame(
            {"WGS_ID": [sample_name], "HV_Database": [DB], "No_HVGs_Found": ["[-/-]"]}
        )
        df.index = [sample_name]
    else:
...
def parse_gamma_pf(gamma_pf_file, sample_name, pf_df):
    ...
    DB = (
        (gamma_pf_file.rsplit("/", 1)[-1]).replace(sample_name, "").rsplit("_")[1]
        + "_"
        + (gamma_pf_file.rsplit("/", 1)[-1])
        .replace(sample_name, "")
        .rsplit("_")[2]
        .strip(".gamma")
        + "([95NT/60]) "
    ...
    if DB == "":
        DB = "Unknown"
    percent_NT_IDs = np.floor(
        gamma_df["Match_Percent"] * 100
    ).tolist()  # round % to whole number
    percent_lengths = np.floor(
        gamma_df["Length_Percent"] * 100
    ).tolist()  # round % to whole number - this is the coverage
    contig_numbers = (
        gamma_df["Contig"].str.replace(sample_name, "").str.split("_").str[1]
    )  # Parse "Contig" column in gamma file
    pf_column_name = gamma_df[
        "Gene"
    ]  # Parse "Gene" column in gamma file to get gene name and accession
    # loop through list of gamma info to combine into "code" for ID%/%cov:contig# and make back into a pandas series
    pf_coverage = [
        "[{:.0f}NT/{:.0f}:#{}]G".format(percent_NT_ID, percent_length, contig_number)
        for percent_NT_ID, percent_length, contig_number in zip(
            percent_NT_IDs, percent_lengths, contig_numbers
        )
    ]
    # Minimum % length required to be included in report, otherwise removed from list
    if bool(
        ...
        for index in sorted(
            index_remove_postion, reverse=True
    ...
    # Minimum % identity required to be included in report, otherwise removed from list
    if bool(
        [percent_NT_ID for percent_NT_ID in percent_NT_IDs if int(percent_NT_ID) < 95]
    ):
        index_remove_postion = [
            n
            for n, percent_NT_ID in enumerate(percent_NT_IDs)
            if int(percent_NT_ID) < 95
        ]  # get index for value removed to remove from other lists (values less than 98)
        percent_NT_IDs = [
            percent_NT_ID
            for percent_NT_ID in percent_NT_IDs
            if int(percent_NT_ID) >= 95
        ]  # filter list to remove values below cutoff (keep those greater than or equal to 98)
...
def parse_mlst(mlst_file, scheme_guess, sample_name):
    ...
    Scheme_list = [
        ...
        [],
        [],
    ]  # create empty list to fill later this would be the MLST_Scheme_1	MLST_1	MLST_Scheme_2	MLST_2
    with open(mlst_file, "r") as f:
        ...
        for rawline in lines:
            ...
            if (
                scheme_guess == "abaum" and "PARALOG" in Scheme
            ):  # supress Paralogs for Acinetobacter_baumannii
                ...
                if (len(set(split_line[5:])) == 1) and split_line[5:][0] == "-":
                    ...
                    alleles = ".".join(
                        split_line[5:]
                    )  # combine all alleles separated by -
                ...
                exclusion_list = [
                    ...
                    "Novel_profile",
                    "Missing_allele",
                    "Novel_allele-PARALOG",
                    "Novel_profile-PARALOG",
                    "Missing_allele-PARALOG",
                ]
                if (
                    DB_ID in Scheme_list[0]
                ):  # check if scheme name is already in the scheme list
                    for i in range(
                        0, len(Scheme_list[0])
                    ):  # loop through list of scheme names
                        if (
                            DB_ID == Scheme_list[0][i]
                        ):  # looking for matching scheme name that was already in the list
                            ...
                            if not any(x in Scheme for x in exclusion_list):
                                Scheme_list[1][i].append(
                                    "ST" + str(Scheme)
                                )  # if there is a value add ST in front of number
                            else:
                                Scheme_list[1][i].append(
                                    Scheme
                                )  # just append what is there
...
def parse_srst2_ar(srst2_file, ar_dic, final_srst2_df, sample_name):
    ...
    coverage = [
        "[{:.0f}NT/{:.0f}]S".format(percent_BP_ID, percent_length)
        for percent_BP_ID, percent_length in zip(percent_BP_IDs, percent_lengths)
    ]
    # Minimum % length required to be included in report, otherwise removed from list
    if bool(
        [
    ...
    # Minimum % identity required to be included in report, otherwise removed from list
    if bool(
        [percent_BP_ID for percent_BP_ID in percent_BP_IDs if int(percent_BP_ID) < 98]
    ):
        index_remove_postion = [
            n
            for n, percent_BP_ID in enumerate(percent_BP_IDs)
            if int(percent_BP_ID) < 98
        ]  # get index for value removed to remove from other lists (values less than 98)
        percent_BP_IDs = [
            percent_BP_ID
            for percent_BP_ID in percent_BP_IDs
            if int(percent_BP_ID) >= 98
        ]  # filter list to remove values below cutoff (keep those greater than or equal to 98)
        # loop through list of indexes to delete and remove them from the other lists so they all match
        for index in sorted(index_remove_postion, reverse=True):
    ...
    if (
        df.empty
    ):  # check if its empty - which would be when nothing is found and/or no hits passed the filter
        ...
        df["WGS_ID"] = sample_name
        df.index = [sample_name]
    final_srst2_df = pd.concat(
        [final_srst2_df, df], axis=0, sort=True, ignore_index=False
    ).fillna("")
...
def Get_Metrics(
    ...
    try:
        (
            ...
            Trim_Q30_R1_percent,
            Trim_Q30_R2_percent,
        ) = get_Q30(trim_stats, raw_stats)
    except FileNotFoundError:
        print("Warning: " + sample_name + "_trimmed_read_counts.txt not found")
        Q30_R1_per = Q30_R2_per = Total_Raw_Seq_bp = Total_Raw_reads = (
            Total_Trimmed_bp
        ) = Paired_Trimmed_reads = Total_Trimmed_reads = Trim_Q30_R1_percent = (
            Trim_Q30_R2_percent
        ) = "Unknown"
    ...
    (
        ...
        scheme_guess_kraken_wt,
        scheme_guess_kraken_trimd,
    ) = get_kraken_info(kraken_trim, kraken_wtasmbld, sample_name)
    try:
        if (
            Total_Trimmed_bp != "Unknown"
        ):  # for -entry CDC_SCAFFOLDS where no reads are present to calculate. For all other scenerios run this portion
            Coverage, Assembly_Length = Calculate_Trim_Coverage(
                Total_Trimmed_bp, quast_report
            )
        elif (
            Total_Trimmed_bp == "Unknown"
        ):  # for -entry CDC_SCAFFOLDS and -entry SCAFFOLDS
    ...
    try:
        ...
        if phoenix_entry == False:  # suppress warning when busco is not run
            print(
                "Warning: short_summary.specific."
                + sample_name
                + ".filtered.scaffolds.fa.txt not found."
            )
    ...
    try:
        QC_result, QC_reason = Checking_auto_pass_fail(
            fairy_file,
            scaffolds_entry,
            Coverage,
            Assembly_Length,
            assembly_ratio_metrics[1],
            assembly_ratio_metrics[0],
            set_coverage,
            Scaffold_Count,
        )
    except FileNotFoundError:
        print(
            "Warning: Possibly coverage and assembly length was not calculated and/or "
            + sample_name
            + "_Assembly_ratio_*.txt not found."
        )
        QC_result = QC_reason = "Unknown"
    try:
        FastANI_output_list, scheme_guess_fastani, fastani_warning = parse_ani(
            fast_ani_file
        )
    except FileNotFoundError:
    ...
    try:
        ...
        df = pd.DataFrame(
            {
                "WGS_ID": [sample_name],
                "No_AR_Genes_Found": ["File not found"],
                "AR_Database": ["GAMMA file not found"],
            }
        )
        df.index = [sample_name]
    ...
    try:
        ...
        df = pd.DataFrame(
            {
                "WGS_ID": [sample_name],
                "No_Plasmid_Markers": ["File not found"],
                "Plasmid_Replicon_Database": ["GAMMA file not found"],
            }
        )
        df.index = [sample_name]
        pf_df = pd.concat([pf_df, df], axis=0, sort=True, ignore_index=False).fillna("")
    try:
        ...
        df = pd.DataFrame(
            {
                "WGS_ID": [sample_name],
                "No_HVGs_Found": ["File not found"],
                "HV_Database": ["GAMMA file not found"],
            }
        )
        df.index = [sample_name]
    ...
    try:
        ...
        if any(x in file_content for x in srst2_failure_checks):
            if (
                "No AR genes found" in file_content
            ):  # no warning needed, but do need to make dataframe empty to keep things moving
                srst2_warning = None
            else:
                srst2_warning = "failed srst2 gene detection."
            df = pd.DataFrame({"WGS_ID": [sample_name]})
            df.index = [sample_name]
            srst2_ar_df = pd.concat(
                [srst2_ar_df, df], axis=0, sort=True, ignore_index=False
            ).fillna("")
        else:
    ...
    ):  # second one for an empty dataframe - srst2 module creates a blank file
        if phoenix_entry == False:  # supress warning when srst2 is not run
            print(
                "Warning: "
                + sample_name
                + "__fullgenes__ResGANNCBI__*_srst2__results.txt not found"
            )
        df = pd.DataFrame({"WGS_ID": [sample_name]})
        df.index = [sample_name]
        srst2_ar_df = pd.concat(
    ...
    try:
        alerts = compile_alerts(
            scaffolds_entry, Coverage, assembly_ratio_metrics[1], gc_metrics[0]
        )
    except:
        alerts = ""
    # try except in the function itself
    kraken_trim_genus, kraken_wtasmbld_genus = parse_kraken_report(
        kraken_trim_report, kraken_wtasmbld_report, sample_name
    )
    ...
    try:
        ...
        if len(Scheme_list[0]) > 1:  # If there is more than one scheme
            if (
                Scheme_list[0][0] < Scheme_list[0][1]
            ):  # this if else is all just to make sure things are printing out in the same order.
                ...
                MLST_scheme_1 = Scheme_list[0][
                    1
                ]  # get 1st scheme name from the list, in this case its the 2nd element
                mlst_types_1 = sorted(Scheme_list[1][1])[::-1]
                MLST_type_1 = ", ".join(mlst_types_1)
                MLST_alleles_1 = ",".join(Scheme_list[2][1])
                MLST_source_1 = ",".join(Scheme_list[3][1])
                MLST_scheme_2 = Scheme_list[0][
                    0
                ]  # get 2nd scheme name from the list, in this case its the first element
            ...
            MLST_type_1 = ", ".join(
                Scheme_list[1][0]
            )  # join together the STs for this one scheme
            MLST_alleles_1 = ",".join(Scheme_list[2][0])
            MLST_source_1 = ",".join(Scheme_list[3][0])
            MLST_scheme_2 = "-"
            MLST_type_2 = "-"
        ...
        MLST_scheme_1 = MLST_scheme_2 = MLST_type_1 = MLST_type_2 = MLST_alleles_1 = (
            MLST_alleles_2
        ) = MLST_source_1 = MLST_source_2 = "Unknown"
    try:
        warnings = compile_warnings(
            ...
            fastani_warning,
            busco_metrics[1],
            FastANI_output_list[1],
            FastANI_output_list[2],
            srst2_warning,
        )
    except:
        warnings = ""
...
def Get_Files(directory, sample_name):
    ...
    kraken_trim = (
        ...
        + "/kraken2_trimd/"
        + sample_name
        + ".kraken2_trimd.top_kraken_hit.txt"
    )
    kraken_trim_report = (
        directory + "/kraken2_trimd/" + sample_name + ".kraken2_trimd.summary.txt"
    )
    kraken_wtasmbld = (
        directory
        + "/kraken2_asmbld_weighted/"
        + sample_name
        + ".kraken2_wtasmbld.top_kraken_hit.txt"
    )
    kraken_wtasmbld_report = (
        directory
        + "/kraken2_asmbld_weighted/"
        + sample_name
        + ".kraken2_wtasmbld.summary.txt"
    )
    ...
    try:
        busco_short_summary = glob.glob(
            directory
            + "/BUSCO/short_summary.specific.*"
            + sample_name
            + ".filtered.scaffolds.fa.txt"
        )[0]
    except IndexError:
        busco_short_summary = (
            directory
            + "/BUSCO/short_summary.specific.blank"
            + sample_name
            + ".filtered.scaffolds.fa.txt"
        )
    try:
        asmbld_ratio = glob.glob(
            directory + "/" + sample_name + "_Assembly_ratio_*.txt"
        )[0]
    except IndexError:
    ...
    try:
        gamma_ar_file = glob.glob(directory + "/gamma_ar/" + sample_name + "_*.gamma")[
            0
        ]
    except IndexError:
        gamma_ar_file = directory + "/gamma_ar/" + sample_name + "_blank.gamma"
    try:
        gamma_pf_file = glob.glob(directory + "/gamma_pf/" + sample_name + "_*.gamma")[
            0
        ]
    except IndexError:
        gamma_pf_file = directory + "/gamma_pf/" + sample_name + "_blank.gamma"
    try:
        gamma_hv_file = glob.glob(directory + "/gamma_hv/" + sample_name + "_*.gamma")[
            0
        ]
    except IndexError:
        gamma_hv_file = directory + "/gamma_hv/" + sample_name + "_blank.gamma"
    try:
        fast_ani_file = glob.glob(
            directory + "/ANI/" + sample_name + "_REFSEQ_*.fastANI.txt"
        )[0]
    except IndexError:
        fast_ani_file = directory + "/ANI/" + sample_name + ".fastANI.txt"
    tax_file = (
        directory + "/" + sample_name + ".tax"
    )  # this file will tell you if kraken2 wtassembly, kraken2 trimmed (reads) or fastani determined the taxa
    try:
        srst2_file = glob.glob(
            directory + "/srst2/" + sample_name + "__fullgenes__*_srst2__results.txt"
        )[0]
    except IndexError:
        srst2_file = (
            directory
            + "/srst2/"
            + sample_name
            + "__fullgenes__blank_srst2__results.txt"
        )
    return (
        trim_stats,
        raw_stats,
...
def Append_Lists(
    ...
    Total_Raw_Seq_bp_L,
    Total_Seq_reads_L,
    Paired_Trimmed_reads_L,
    Total_trim_Seq_reads_L,
    Trim_kraken_L,
    Asmbld_kraken_L,
    Coverage_L,
    Assembly_Length_L,
...
def add_srst2(ar_df, srst2_ar_df):
    ...
    common_cols = ar_df.columns.intersection(
        srst2_ar_df.columns
    ...
    for col in common_cols:
        if col != "WGS_ID":
            ar_combined_df[col] = (
                srst2_ar_df[col].map(str) + ":" + ar_df[col]
            ).replace(":", "")
            ar_combined_df[col] = ar_combined_df[col].map(
                lambda x: str(x).lstrip(":").rstrip(":")
            )  # clean up : for cases where there isn't a gamma and srst2 for all rows
            ar_combined_df = (
                ar_combined_df.copy()
            )  # defragment to correct "PerformanceWarning: DataFrame is highly fragmented."
        else:
    ...
    ar_combined_ordered_df = pd.concat(
        [ar_combined_ordered_df, ar_combined_df[["AR_Database", "WGS_ID"]]],
        axis=1,
        sort=False,
    )  # first adding back in ['AR_Database', 'WGS_ID']
    ar_drugs_list = ar_combined_df.columns.str.extract(
        ".*\((.*)\).*"
    ).values.tolist()  # get all ar drug names form column names
    sorted_list = sorted(
        list(set([str(drug) for sublist in ar_drugs_list for drug in sublist]))
    )  # get unique drug names (with set) and sort list
    sorted_drug_names = [
        x for x in sorted_list if x != "nan"
    ]  # get unique drug names (with set) and drop nan that comes from WGS_ID column and sort
    ...
    for drug in sorted_drug_names:
        drug = "(" + drug + ")"
        column_list = sorted(
            [col for col in ar_combined_df.columns if drug in col]
        )  # get column names filtered for each drug name
        ar_combined_ordered_df = pd.concat(
            [ar_combined_ordered_df, ar_combined_df[column_list]], axis=1, sort=False
        )  # setting column's order by combining dataframes
...
def big5_check(final_ar_df):
    ...
    for (
        ...
        all_genes
    ):  # loop through each gene in the dataframe of genes found in all isolates
        if gene == "No_AR_Genes_Found":
            ...
            if gene_name.startswith("blaOXA"):  # check for complete blaOXA match
                [
                    ...
                    for big5_keep_gene in blaOXA_48_like
                    if gene_name == big5_keep_gene
                ]
                ...
                for big5 in big5_keep:
                    if search(
                        big5, gene_name
                    ):  # search for big5 gene substring in the gene name
                        columns_to_highlight.append(gene_name + "_(" + drug)
    ...
    for bad_gene in big5_drop:
        ...
        [
            columns_to_highlight.remove(gene)
...
def write_to_excel(
    ...
    for idx, col in enumerate(df):  # loop through all columns
        ...
        if (
            col == "Parent_Folder"
        ):  # name is real long due to path so just making the width of the column header
    ...
    cell_format_light_blue = workbook.add_format(
        {"bg_color": "#ADD8E6", "font_color": "#000000", "bold": True}
    )
    cell_format_grey_blue = workbook.add_format(
        {"bg_color": "#72A0C1", "font_color": "#000000", "bold": True}
    )
    cell_format_green_blue = workbook.add_format(
        {"bg_color": "#A3C1AD", "font_color": "#000000", "bold": True}
    )
    cell_format_green = workbook.add_format(
        {"bg_color": "#ACE1AF", "font_color": "#000000", "bold": True}
    )
    cell_format_lightgrey = workbook.add_format(
        {"bg_color": "#D5D8DC", "font_color": "#000000", "bold": True}
    )
    cell_format_grey = workbook.add_format(
        {"bg_color": "#AEB6BF", "font_color": "#000000", "bold": True}
    )
    cell_format_darkgrey = workbook.add_format(
        {"bg_color": "#808B96", "font_color": "#000000", "bold": True}
    )
    # Headers
    ...
    worksheet.merge_range(
        ...
        qc_max_col,
        0,
        (qc_max_col + ar_gene_count - 1),
        "Antibiotic Resistance Genes",
        cell_format_lightgrey,
    )
    worksheet.merge_range(
        0,
        (qc_max_col + ar_gene_count),
        0,
        (qc_max_col + ar_gene_count + hv_gene_count - 1),
        "Hypervirulence Genes^^",
        cell_format_grey,
    )
    worksheet.merge_range(
        0,
        (qc_max_col + ar_gene_count + hv_gene_count),
        0,
        (qc_max_col + ar_gene_count + pf_gene_count + hv_gene_count - 1),
        "Plasmid Incompatibility Replicons^^^",
        cell_format_darkgrey,
    )
    # making WGS IDs bold
    ...
    yellow_format = workbook.add_format(
        {"bg_color": "#FFEB9C", "font_color": "#000000"}
    )  # Light yellow fill with black text.
    red_format = workbook.add_format(
        {"bg_color": "#F5B7B1", "font_color": "#000000"}
    )  # red fill with black text.
    orange_format = workbook.add_format(
        {"bg_color": "#F5CBA7", "font_color": "#000000", "bold": True}
    )
    orange_format.set_border(
        1
    )  # add back border so it matches the rest of the column names
    orange_format_nb = workbook.add_format(
        {"bg_color": "#F5CBA7", "font_color": "#000000", "bold": False}
    )
    ...
    worksheet.conditional_format(
        "M3:M" + str(max_row + 2),
        {
            "type": "cell",
            "criteria": "<",
            "value": str(set_coverage),
            "format": yellow_format,
        },
    )
    worksheet.conditional_format(
        "M3:M" + str(max_row + 2),
        {"type": "cell", "criteria": ">", "value": 100.00, "format": yellow_format},
    )
    ...
    for column in ar_df.columns:
        for gene in columns_to_highlight:
            if column == gene:  # if the column is one of the big 5 genes to highlight
                col_adjustment = (
                    column_count + qc_max_col - 1
                )  # adjust starting place to account for qc columns
                cell = xl_rowcol_to_cell(
                    1, col_adjustment
                )  # Gets the excel location like A1
    ...
    worksheet.write(
        "A" + str(max_row + 4),
        "Cells in YELLOW denote isolates outside of "
        + str(set_coverage)
        + "-100X coverage",
        yellow_format,
    )
    worksheet.write(
        "A" + str(max_row + 5),
        "Cells in ORANGE denote “Big 5” carbapenemase gene (i.e., blaKPC, blaNDM, blaOXA-48-like, blaVIM, and blaIMP) or an acquired blaOXA gene, please confirm what AR Lab Network HAI/AR WGS priority these meet.",
        orange_format_nb,
    )
    worksheet.write(
        "A" + str(max_row + 6),
        "Cells in RED denote isolates that failed one or more auto failure triggers (cov < 30, assembly ratio stdev > 2.58, assembly length < 1Mbps)",
        red_format,
    )
    ...
    worksheet.write(
        "A" + str(max_row + 7),
        "^Using Antibiotic Resistance Gene database "
        + ar_db
        + " (ResFinder, ARG-ANNOT, NCBI Bacterial Antimicrobial Resistance Reference Gene Database) using output thresholds ([98AA/90]G:[98NT/90]S); gene matches from S:(SRST2) with [%Nuc_Identity, %Coverage], or from G:(GAMMA) with [%Nuc_Identity, %AA_Identity,  %Coverage]; GAMMA gene matches indicate associated contig.",
        no_bold,
    )
    worksheet.write(
        "A" + str(max_row + 8),
        "^^Using CDC-compiled iroB, iucA, peg-344, rmpA, and rmpA2 hypervirulence gene database ( "
        + hv_db
        + " ); gene matches noted with [%Nuc_Identity, %AA_Identity,  %Coverage].",
        no_bold,
    )
    worksheet.write(
        "A" + str(max_row + 9),
        "^^^Using the plasmid incompatibility replicons plasmidFinder database ( "
        + pf_db
        + " ) using output thresholds [95NT/60]; replicon matches noted with [%Nuc_Identity, %Coverage].",
        no_bold,
    )
    worksheet.write(
        "A" + str(max_row + 10),
    ...
    with open(control_file, "r") as controls:
        ...
        for line in controls:
            ...
            if new_sample_name != old_sample_name:
                final_df["WGS_ID"] = final_df["WGS_ID"].replace(
...
def create_samplesheet(directory):
    ...
    skip_list_a1 = glob.glob(
        directory + "/*_GRiPHin_Summary.*"
    )  # for if griphin is run on a folder that already has a report in it
    skip_list_a2 = glob.glob(directory + "/*_comparison")  # for comparinator script
    skip_list_a = skip_list_a1 + skip_list_a2
    skip_list_a = [
        gene.split("/")[-1] for gene in skip_list_a
    ]  # just get the excel name not the full path
    skip_list_b = [
    ...
    try:  # if there are numbers in the name then use that to sort
        dirs_sorted = sorted(
            dirs_cleaned, key=lambda x: int("".join([i for i in x if i.isdigit()]))
        )
...
def sort_samplesheet(samplesheet):
    ...
    try:  # if there are numbers in the name then use that to sort
        samples_sorted = sorted(
            samples, key=lambda x: int("".join([i for i in x if i.isdigit()]))
        )
    except:  # if no numbers then use only alphabetically
...
def convert_excel_to_tsv(output):
    ...
    data_xlsx = pd.read_excel(
        output_file + ".xlsx", "Sheet1", index_col=None, header=[1]
    )
...
def main():
    ...
    pf_df = (
        pd.DataFrame()
    )  # create another empty dataframe to fill later for Plasmid markers
    hv_df = (
        pd.DataFrame()
    )  # create another empty dataframe to fill later for hypervirulence genes
    ...
    if (args.samplesheet == None) and (
        args.directory == None
    ):  # if no directory give AND no sample sheet given exit
        sys.exit(
            CRED
            + "You MUST pass EITHER a samplesheet or a top directory of PHoeNIx output to create one.\n"
            + CEND
        )
    ...
    with open(samplesheet) as csv_file:
        ...
        for row in csv_reader:
            ...
            (
                ...
                tax_file,
                srst2_file,
            ) = Get_Files(directory, sample_name)
    ...
    final_df, ar_max_col, columns_to_highlight, final_ar_df, pf_db, ar_db, hv_db = (
        Combine_dfs(df, ar_df, pf_df, hv_df, srst2_ar_df, args.phoenix)
    )
