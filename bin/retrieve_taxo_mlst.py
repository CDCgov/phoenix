    ...
    for path in fullpath:
        ...
        try:  # handling for cases where no .tax file is created
            ...
            for tax_level in list(file_content.values):
                file_dict[tax_level[0].split(":")[0]] = (
                    tax_level[0].split("\t")[1].strip()
                )
    ...
    for path in fullpath:  # if a griphin samplesheet was passed
        try:  # handling for cases where no _combined.tsv file is created
            ...
            file_content = pd.read_csv(
                ...
                delimiter="\t",
                header=0,
                usecols=["Sample", "Source", "Pulled_on", "Database", "ST"],
            )
            if file_content.empty:
                print(
                    path.split("/")[len(path.split("/")) - 1] + ": No mlst info found"
                )
            else:
                # check "PARALOG"
                result = (
                    file_content.astype(str)
                    .apply(lambda x: x.str.contains("PARALOG", case=False))
                    .values.flatten()
                )
                if result.any():
                    file_content = file_content.drop(
                        file_content.index[file_content["ST"].str.contains("PARALOG")]
                    ).reindex()
                # check "Oxford" "Pasteur"
                indices = file_content.index[
                    file_content["Database"].str.contains("Oxford")
                    | file_content["Database"].str.contains("Pasteur")
                    | file_content["Database"].str.contains("Achtman")
                ]
                tmpdict = tools.rearrange_oxford_pasteur(file_content, indices)
                isolate_mlst[file_content["Sample"][0].strip()] = tmpdict[
                    file_content["Sample"][0].strip()
                ]
        except FileNotFoundError:
            isolate_mlst[file_content["Sample"][0].strip()] = ""
    return isolate_mlst
        ...
        try:
            ...
            if file_content.empty:
                ...
                if file_content["Database"][0] == "-":
                    file_content = file_content.drop(
                        file_content[(file_content["Database"] == "-")].index
                    )
                # check "Novel", "-", "Missing"
                result = (
                    file_content.astype(str)
                    .apply(lambda x: x.str.contains("Novel|Missing|-", case=False))
                    .values.flatten()
                )
                if result.any():
                    ...
                    file_content = file_content.drop(
                        file_content.index[
                            file_content["ST"].str.contains(
                                "Novel|Missing|-", case=False
                            )
                        ]
                ...
                if file_content.shape[0] == 0:
                    ...
                    isolate_mlst[file_content["Sample"][0].strip()] = tmp_dict[
                        file_content["Sample"][0].strip()
                    ]
        except FileNotFoundError:
            isolate_mlst[isolate_name] = ""
...
def retrieve_instrument_model(fullpath):
    ...
    for path in fullpath:
        ...
        seq_machine_model = (
            pd.read_csv(fastq_path, compression="gzip", nrows=1)
            .columns.values[0]
            .split(":")[0]
            .replace("@", "")
            .strip()
    ...
    print(
        "The following isolates do not have sequence machine model information provided:"
    )
