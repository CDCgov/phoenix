...
def parseArgs(args=None):
    parser = argparse.ArgumentParser(
        description="Script to check MLST types for duplicate alleles and implications on final typing"
    )
    ...
    parser.add_argument(
        ...
        "--taxonomy",
        required=True,
        help="Location of taxonomy file to pull right scheme",
    )
    parser.add_argument(
        "-d",
        "--mlst_database",
        required=True,
        help="Path to mlst db of db/pubmlst/schemes/ format",
    )
    parser.add_argument(
...
def do_MLST_check(input_MLST_line_tuples, taxonomy_file, mlst_db_path):
        ...
        if original_line is not None and original_line != "":
            ...
            # allele_list=[['1'], ['3'], ['189','3'], ['2'], ['2'], ['96','107'], ['3']]
                ...
                if len(original_items) > 7:
                    ...
                    expanded_allele_list.append(
                        [sample, db_name, "-", 1, ["-"], ["-"], "reads", pull_date]
                    )
    ...
    for i in range(0, len(catted_scheme_list)):
        for j in range(0, len(catted_scheme_list)):
            ...
            if collections.Counter(catted_scheme_list[i][5]) == collections.Counter(
                catted_scheme_list[j][5]
            ):
                print("SAME!!!!", i, j)
                if catted_scheme_list[min(i, j)][6] != catted_scheme_list[max(i, j)][6]:
                    ...
                    if catted_scheme_list[min(i, j)][6] == "assembly":
                        ...
                        print("i")
                        print(
                            "Should never have something that is not assembly, reads, or assembly/reads"
                        )
                else:
                    if (
                        catted_scheme_list[min(i, j)][1]
                        != catted_scheme_list[max(i, j)][1]
                    ):
                        print("Different dbs")
                    else:
    ...
    #######
        ...
        for original_scheme in catted_scheme_list:
            ...
            if check:
                ...
                if (
                    collections.Counter(original_scheme[5])
                    == collections.Counter(["-", "-", "-", "-", "-", "-", "-"])
                    or collections.Counter(original_scheme[5])
                    == collections.Counter(["-", "-", "-", "-", "-", "-", "-", "-"])
                    or collections.Counter(original_scheme[5])
                    == collections.Counter(["-"])
                ):
                ...
                if lookup_allele_profile:
                    ...
                    profile_file = (
                        mlst_db_path + "/pubmlst/" + db_name + "/" + db_name + ".txt"
                    )
    ...
    for i in checked_schemes:
        ...
        if (
            i[1] == "abaumannii"
            or i[1] == "abaumannii(Oxford)"
            or i[1] == "Acinetobacter_baumannii#1"
        ):
            if len(i[5]) > 1:
        ...
        while i >= 0:
            ...
            for allele in novel_allele_sets[i][5]:
                if "*" in allele or "?" in allele or "~" in allele:  # or '-' in allele:
                    ...
                    raw_1_allele = (
                        allele.replace("*", "").replace("?", "").replace("~", "")
                    )
                ...
                if i == j:
                    ...
                    if primary_db == novel_allele_sets[j][1]:
                        ...
                        for allele in novel_allele_sets[j][5]:
                            if (
                                "*" in allele or "?" in allele or "~" in allele
                            ):  # or '-' in allele:
                                # Mark allele as found, but a mismatch
                                raw_2_allele = (
                                    allele.replace("*", "")
                        ...
                        if primary_alleles == secondary_alleles:
                            ...
                            if primary_source == "assembly":
                                ...
                                if secondary_source == "assembly":
                                    print(
                                        "Weird, shouldnt have both novel allele sets in one database come from assembly MLST"
                                    )
                                    # Not as weird, contamination testing this comes up, so a good way to catch?
                                    new_source = "assembly"
                                elif secondary_source == "assembly/reads":
                                    print(
                                        "Weird, shouldnt have both novel allele sets in one database come from assembly MLST (plus a confirmation srst2)"
                                    )
                                    # Not as weird, contamination testing this comes up, so a good way to catch?
                                    ...
                                    print(
                                        "Weird, phx doesn't know what the secondary source is"
                                    )
                                ...
                                if secondary_source == "assembly":
                                    ...
                                    print(
                                        "Weird, shouldnt have both novel allele sets in one database come from srst2 MLST (plus a confirmation assembly MLST)"
                                    )
                                    # Not as weird, contamination testing this comes up, so a good way to catch?
                                    new_source = "assembly/reads"
                                elif secondary_source == "reads":
                                    print(
                                        "Weird, shouldnt have both novel allele sets in one database come from srst2"
                                    )
                                    # Not as weird, contamination testing this comes up, so a good way to catch?
                                    new_source = "reads"
                                else:
                                    print(
                                        "Weird, dont know what the secondary source is"
                                    )
                                # secondary_is_srst2=True
                            new_allele_list = []
                            for k in range(0, len(primary_alleles)):
                                if primary_alleles[k] == "Mismatch":
                                    ...
                                    if (
                                        novel_allele_sets[i][5][k]
                                        == novel_allele_sets[j][5][k]
                                    ):
                                        ...
                                        if primary_is_srst2:
                                            ...
                                            print(
                                                "Weird, there is no srst2 found when comparing....these are all assembly MLSTs"
                                            )
                                else:
                                    new_allele_list.append(novel_allele_sets[i][5][k])
                            new_entry = [
                                novel_allele_sets[i][0],
                                novel_allele_sets[i][1],
                                novel_allele_sets[i][2],
                                novel_allele_sets[i][3],
                                novel_allele_sets[i][4],
                                new_allele_list,
                                new_source,
                                novel_allele_sets[i][7],
                            ]
    ...
    with open(outfile, "w") as writer:
        writer.write(
            "Sample\tSource\tPulled_on\tDatabase\tST\tlocus_1\tlocus_2\tlocus_3\tlocus_4\tlocus_5\tlocus_6\tlocus_7\tlocus_8\tlocus_9\tlocus_10\n"
        )
        if len(checked_and_deduped_schemes) == 0:
            ...
            for i in checked_and_deduped_schemes:
                ...
                if (
                    (i[2].isnumeric() or "-PARALOG" in i[2] or "Novel_profile" in i[2])
                    and i[2] != "-"
                    and all_Types_are_complete != "False"
                ):
                    # print(i[2], "= good")
