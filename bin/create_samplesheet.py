...
def parseArgs(args=None):
    parser = argparse.ArgumentParser(
        description="Script to generate a samplesheet with sample,directory columns."
    )
    parser.add_argument(
        "-d",
        "--directory",
        ...
        required=False,
        dest="directory",
        help="Will create a samplesheet for all samples in the directory (expects PHoeNIx style directory structure).",
    )
...
def create_samplesheet(directory):
    ...
    with open("GRiPHin_samplesheet_created.csv", "w") as samplesheet:
        ...
        skip_list_a = glob.glob(
            directory + "/*_GRiPHin_Report.xlsx"
        )  # for if griphin is run on a folder that already has a report in it
        skip_list_a = [
            item.split("/")[-1] for item in skip_list_a
...
def main():
    ...
    if args.directory != None:
        ...
        sys.exit(
            CRED
            + "You MUST pass a stop directory of PHoeNIx output to create a samlesheet.\n"
            + CEND
        )
