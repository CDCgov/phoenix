...
def parseArgs(args=None):
    parser = argparse.ArgumentParser(
        description="Script to generate a combined GRiPHin summary excel sheet"
    )
    parser.add_argument(
        "-o",
        "--out",
...
def separate_column_type(excl_merged, df):
    ...
    pf_col_list.insert(
        0, pf_col_list.pop(pf_col_list.index("Plasmid_Replicon_Database"))
    )
...
def fix_ar_col_order(col_list):
    ...
    for drug in ar_drugs_list:
        drug = "(" + drug + ")"
        ar_column_list = sorted(
            [col for col in col_list if drug in col]
...
def write_excel(
    ...
    if (
        output_file != None and output_file != "GRiPHin_Summary.xlsx"
    ):  # check that its not "GRiPHin_Summary.xlsx"
        writer = pd.ExcelWriter(
            (output_file + "_GRiPHin_Summary.xlsx"), engine="xlsxwriter"
        )
    else:
