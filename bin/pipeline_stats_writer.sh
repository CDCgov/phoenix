...
if [[ "${run_type}" == "all" ]]; then
  ...
  # if [[ -s "${removed_adapter_R2}" ]]; then
 	...
 	if [[ -s "${kraken2_trimd_summary}" ]]; then
 		...
 		unclass=$(head -n 1 "${kraken2_trimd_summary}" | cut -d' ' -f2)
 		#true_unclass=$(head -n 1 "${OUTDATADIR}/kraken/preAssembly/${sample_name}_kraken_summary_paired.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
 		domain=$(sed -n '2p' "${kraken2_trimd_summary}" | cut -d' ' -f2)
 		genuspre=$(sed -n '7p' "${kraken2_trimd_summary}" | cut -d' ' -f3)
 		speciespre=$(sed -n '8p' "${kraken2_trimd_summary}" | cut -d' ' -f3)
 		speciesprepercent=$(sed -n '8p' "${kraken2_trimd_summary}" | cut -d' ' -f2)
    if [[ "${unclass}" = "UNK" ]]; then
      unclass=0
 ...
 if [[ -s "${kraken2_asmbled_summary}" ]]; then
 	# Extracts many elements of the summary file to report unclassified and species classified reads and percentages
 	unclass=$(head -n 1 "${kraken2_asmbled_summary}" | cut -d' ' -f2)
 	#true_unclass=$(head -n 1 "${OUTDATADIR}/kraken2/postAssembly/${sample_name}_kraken_summary_assembled.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
 	domain=$(sed -n '2p' "${kraken2_asmbled_summary}" | cut -d' ' -f2)
 	genuspost=$(sed -n '7p' "${kraken2_asmbled_summary}" | cut -d' ' -f3)
 	speciespost=$(sed -n '8p' "${kraken2_asmbled_summary}" | cut -d' ' -f3)
 	speciespostpercent=$(sed -n '8p' "${kraken2_asmbled_summary}" | cut -d' ' -f2)
  genuspostpercent=$(sed -n '7p' "${kraken2_asmbled_summary}" | cut -d' ' -f2)
  if [[ "${unclass}" = "UNK" ]]; then
    unclass=0
    unclass_string="UNKNOWN"
  else
 ...
 if [[ -s "${kraken2_weighted_summary}" ]]; then
 	# Extracts many elements of the summary file to report unclassified and species classified reads and percentages
 	unclass=$(head -n 1 "${kraken2_weighted_summary}" | cut -d' ' -f2)
 	#true_unclass=$(head -n 1 "${OUTDATADIR}/kraken2/postAssembly/${sample_name}_kraken_summary_assembled_BP.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
 	domain=$(sed -n '2p' "${kraken2_weighted_summary}" | cut -d' ' -f2)
 	genusweighted=$(sed -n '7p' "${kraken2_weighted_summary}" | cut -d' ' -f3)
 	speciesweighted=$(sed -n '8p' "${kraken2_weighted_summary}" | cut -d' ' -f3)
 	speciesweightedpercent=$(sed -n '8p' "${kraken2_weighted_summary}" | cut -d' ' -f2)
  genusweightedpercent=$(sed -n '7p' "${kraken2_weighted_summary}" | cut -d' ' -f2)
  if [[ "${unclass}" = "UNK" ]]; then
    unclass=0
    unclass_string="UNKNOWN"
  else
 ...
 if [[ -s "${formatted_fastANI}" ]]; then
  ...
  #    primary_mlst_file=""
 	...
 	# 			break
 	# 		elif [[ ${mlst_file} = *"${sample_name}_Oxford.mlst" ]] || [[ ${mlst_file} = *"${sample_name}_Achtman.mlst" ]] || [[ ${mlst_file} = *"${sample_name}_mabscessus.mlst" ]]; then
 	# 			:
  ...
  #      printf "%-30s: %-8s : %s\\n" "MLST" "FAILED" "no scheme found, check if genus/species exists in MLST DB"
  #    elif [[ -n "${primary_mlst_file}" ]]; then
  #      info=$(head -n1 "${primary_mlst_file}")
  #   		mlstype=$(echo "${info}" | cut -d'	' -f3)
  #      mlstdb=$(echo "${info}" | cut -d'	' -f2)
  #   		mlstdb_filelabel=$(echo "${primary_mlst_file}" | rev | cut -d'.' -f2 | cut -d'_' -f1 | rev)
  #   		if [ "${mlstdb}" = "-" ]; then
  ...
  #   				fi
  #   			else
  #   				printf "%-30s: %-8s : %s\\n" "MLST" "FAILED" "no scheme found, check upstream as no genus/species has been assigned"
  #   			fi
  #   		elif [ "${mlstype}" = "-" ] || [ "${mlstype}" = "A-SUB" ] || [ "${mlstype}" = "P-SUB" ]; then
  #   			printf "%-30s: %-8s : %s\\n" "MLST_${mlstdb_filelabel}" "WARNING" "No type found, possibly new type?"
  #   			report_info=$(echo "${info}" | cut -d' ' -f2-)
  #   			if [[ "${status}" = "SUCCESS" ]] || [[ "${status}" = "ALERT" ]]; then
  #   				status="WARNING"
  #   			fi
  #   		elif [ "${mlstype}" = "AU" ]; then
  #   			printf "%-30s: %-8s : %s\\n" "MLST_${mlstdb_filelabel}" "FAILED" "1+ allele is missing, cant determine ST type in ${mlstdb}(${mlstdb_filelabel})"
  #   			status="FAILED"
  #   		else
  #   			printf "%-30s: %-8s : %s\\n" "MLST_${mlstdb_filelabel}" "SUCCESS" "TYPE is ${mlstype} from ${mlstdb}(${mlstdb_filelabel})"
  #   		fi
  # 	  else
  # 		   printf "%-30s: %-8s : %s\\n" "MLST_${mlstdb_filelabel}" "FAILED" "${sample_name}_${mlstdb_filelabel}.mlst does not exist"
  # 		   status="FAILED"
  # 	  fi
  ...
  #   					printf "%-30s: %-8s : %s\\n" "MLST_OX" "WARNING" "No type found, possibly new type?"
  #   					report_info=$(echo "${info}" | cut -d' ' -f2-)
  #   					if [[ "${status}" = "SUCCESS" ]] || [[ "${status}" = "ALERT" ]]; then
  #   						status="WARNING"
  #   					fi
  #   				elif [ "${mlstype}" = "AU" ]; then
  #   					printf "%-30s: %-8s : %s\\n" "MLST_OX" "FAILED" "1+ allele is missing, cant determine ST type in ${mlstdb}(Oxford)"
  #   					status="FAILED"
  #   				else
  #   					printf "%-30s: %-8s : %s\\n" "MLST_OX" "SUCCESS" "TYPE is ${mlstype} from ${mlstdb}(Oxford)"
  #   				fi
  #   			else
  #   				echo "Not reporting as name and analyis expected do not match"
  #   			fi
  #      else
  #        printf "%-30s: %-8s : %s\\n" "MLST_OX" "FAILED" "${sample_name}_Oxford.mlst does not exist"
  #   		fi
  ...
  #   				if [ "${mlstype}" = "A-SUB" ] || [ "${mlstype}" = "P-SUB" ] || [ "${mlstype}" = "-" ]; then
  #   					printf "%-30s: %-8s : %s\\n" "MLST_ACHT" "WARNING" "No type found, possibly new type?"
  #   					report_info=$(echo "${info}" | cut -d' ' -f2-)
  #   					if [[ "${status}" = "SUCCESS" ]] || [[ "${status}" = "ALERT" ]]; then
  #   						status="WARNING"
  #   					fi
  #   				elif [ "${mlstype}" = "AU" ]; then
  #   					printf "%-30s: %-8s : %s\\n" "MLST_ACHT" "FAILED" "1+ allele is missing, cant determine ST type in ${mlstdb}(Achtman)"
  #   					status="FAILED"
  #   				else
  #   					printf "%-30s: %-8s : %s\\n" "MLST_ACHT" "SUCCESS" "TYPE is ${mlstype} from ${mlstdb}(Achtman)"
  #   				fi
  #   			else
  #   				echo "Not reporting as name and analyis expected do not match"
  #   			fi
  #      else
  #        printf "%-30s: %-8s : %s\\n" "MLST_ACHT" "FAILED" "${sample_name}_Achtman.mlst does not exist"
  #   		fi
  ...
  #   				if [ "${mlstype}" = "A-SUB" ] || [ "${mlstype}" = "P-SUB" ] || [ "${mlstype}" = "-" ]; then
  #   					printf "%-30s: %-8s : %s\\n" "MLST_MAB" "WARNING" "No type found, possibly new type?"
  #   					report_info=$(echo "${info}" | cut -d' ' -f2-)
  #   					if [[ "${status}" = "SUCCESS" ]] || [[ "${status}" = "ALERT" ]]; then
  #   						status="WARNING"
  #   					fi
  #   				elif [ "${mlstype}" = "AU" ]; then
  #   					printf "%-30s: %-8s : %s\\n" "MLST_MAB" "FAILED" "1+ allele is missing, cant determine ST type in ${mlstdb}(Mabscessus)"
  #   					status="FAILED"
  #   				else
  #   					printf "%-30s: %-8s : %s\\n" "MLST_MAB" "SUCCESS" "TYPE is ${mlstype} from ${mlstdb}(Mabscessus)"
  #   				fi
  #   			else
  #   				echo "Not reporting as name and analyis expected do not match"
  #   			fi
  #      else
  #        printf "%-30s: %-8s : %s\\n" "MLST_MAB" "FAILED" "${sample_name}_mabscessus.mlst does not exist"
  #   		fi
  # 	  fi
  #  else
  #    printf "%-30s: %-8s : %s\\n" "MLST" "FAILED" "${OUTDATADIR}/${sample_name}/MLST does not exist"
  #    status="FAILED"
  #  fi
