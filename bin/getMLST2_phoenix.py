...
def main():
	...
	#with open('dbs.xml', 'wb') as file:
	#	file.write(response.context)
	#urllib.request.urlretrieve(args.repository_url, 'dbs.xml')
	filedata = urllib2.urlopen(args.repository_url)
	datatowrite = filedata.read()
	with open('dbases.xml', 'wb') as f:
		f.write(datatowrite)
	#docFile = url.urlopen(args.repository_url)
	doc = xml.parse('dbases.xml')
	...
	profile_filename = profile_path.split('/')[-1].replace("profiles_csv",species_name_underscores+"_profiles.csv")
	...
	#print("    srst2 --output test --input_pe *.fastq.gz --mlst_db " + species_name_underscores + '.fasta'+"	--mlst_definitions " + format(profile_filename)+"--mlst_delimiter '" + m[1] + "'"")
	with open(species_name_underscores+"_getMLST_out.txt", "w") as out_getmlst:
		print("DB:"+species_name_underscores+"	defs:"+format(profile_filename)+"	del:'"+m[1]+"'")
		out_getmlst.write("DB:"+species_name_underscores+"	defs:"+format(profile_filename)+"	del:'"+m[1]+"'\n")
