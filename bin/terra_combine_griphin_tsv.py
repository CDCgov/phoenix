    ...
    for file in file_list:
        ...
        try:  # if there are no failures due to coverage then the coverage is unknown
            ...
            if not cell_val.empty:
                ...
                print(
                    "Warning: the coverage used to run {} could not be determined!".format(
                        file
                    )
                )
...
def write_combined_tsv(df, output):
    if (
        output != None and output != "GRiPHin_Summary.tsv"
    ):  # check that its not "GRiPHin_Summary.tsv"
...
def main():
    ...
    if len(file_list) == 0:
        print(
            "Error: No GRiPHin_Summary.tsv files were found using *GRiPHin_Summary.tsv!"
        )
        exit()
