...
import bin.convert_mlst_to_pubMLST_old2 as convert_mlst_to_pubMLST_old2
...
def parseArgs(args=None):
	...
	parser.add_argument('-d', '--docfile', dest="docfile", required=False, help='dbases.xml file')
...
def getSpeciesInfo(species_node, species, exact):
	...
	this_name = getText(species_node) # .encode('utf-8') Jills edit
	store = False
...
def download_MLST_files(tax_to_download, docFile):
	...
	subprocess.run(["wget", "--secure-protocol=TLSv1_3", "-O", profile_filename, species_info.profiles_url.decode() ])
...
def do_MLST_check(input_MLST_line_tuples, taxonomy_file, docFile):
	...
	#					   print(expanded_allele_list)
			...
			#allele_list=[['1'], ['3'], ['189','3'], ['2'], ['2'], ['96','107'], ['3']]
				...
				db_name=convert_mlst_to_pubMLST_old2.back_2_MLST(pubmlst_db_name)
		...
		for original_scheme in catted_scheme_list:
			...
			if check:
				...
				if lookup_allele_profile:
					new_db_name=convert_mlst_to_pubMLST_old2.convert(db_name)
