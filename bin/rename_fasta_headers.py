...
def parseArgs(args=None):
    parser = argparse.ArgumentParser(
        description="Script to rename contigs in assemblies"
    )
    ...
    parser.add_argument(
        "--reverse", help="returns formatted header to original", action="store_true"
    )
    parser.add_argument(
...
def unicycler_rename(input_file, name, output):
    ...
    for record in SeqIO.parse(input_file, "fasta"):
        record.id = (
            ...
            + str(record.id)
            + "_length_"
            + str(record.description.split(" ")[1].replace("length=", ""))
        )
        record.description = ""
        sequences.append(record)
    SeqIO.write(sequences, output, "fasta")
...
def flye_rename(input_file, name, output):
    ...
    for record in SeqIO.parse(input_file, "fasta"):
        record.id = (
            name + "_" + record.id.split("_")[1] + "_length_" + str(len(record.seq))
        )
        # record.id = record.id.replace("contig",name)
        record.description = ""
        sequences.append(record)
...
def main():
    ...
    if assembler == "spades":
        ...
        assembler == "correct name"
    ):  # if the name looks like the correct scheme then just rename and move on
        ...
        print(
            "ERROR: The assembler used could not be determined. PHoeNIx supports assemblies from either Skesa, Shovill, SPAdes, Unicycler,Trycycler and Flye. If you used one of these open a github issue to report the problem."
        )
