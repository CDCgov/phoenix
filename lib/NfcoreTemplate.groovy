...
class NfcoreTemplate {
    ...
    // Construct and send completion email
    //
    public static void centar_email(workflow, params, summary_params, projectDir, log, multiqc_report=[]) {
        ...
        def output_d // Declare the variable at the top
        // Write summary e-mail HTML to a file
        if (params.outdir != "${workflow.launchDir}/phx_output") {
            output_d = new File("${params.outdir}/centar_pipeline_info/")  // Default path
        } else if (params.indir != null) {
            output_d = new File("${params.indir}/centar_pipeline_info/")  // Save to params.indir if defined
        } else if (params.input != null) {
            output_d = new File("${workflow.launchDir}/centar_pipeline_info/")
        }
