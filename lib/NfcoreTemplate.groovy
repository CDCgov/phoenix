    ...
    public static void summary(workflow, params, log) {
        ...
        if (workflow.success) {
            if (workflow.stats.ignoredCount == 0) {
                ...
                log.info "${colors.biyellow} \n***Reminder: If your lab has received funds (ELC, SHARP, etc.) to sequence isolates under the HAI/AR component of the AR Lab Network, please upload relevant sequence files to CDC's NCBI HAI-Seq Umbrella Project (ID 531911).\n ${colors.reset}"
                log.info "${colors.biyellow} \nAlso, update any relevant Alerts records with the HAI WGS ID and SRR ID, within 7-10 business days from sequencing completion.***\n ${colors.reset}"
            } else {
